...
export default function RC_sim()
{
    ...
    const set_graphdata = () => {
        ...
        if(capacitance == 0 || resistance == 0 || interval == 0)
            return; //0除算、無限ループが発生するのを防止

        const number_of_data = time / interval;
        const max_points = 100; // グラフに表示する最大データポイント数（サンプリング用）
        let step : number = 0;
        const between = Math.ceil(number_of_data / max_points);     //サンプリング間隔
        let v_cache = 0;
        ...
        for(let now : number = interval; now <= time+interval; now=Number((now+interval).toFixed(5)))
        {
            ...
            const exact = voltage * (1 - Math.exp(-1.0 * now / (capacitance * resistance)));    //厳密式での計算
            
            //step%between ... intervalが短くデータが多すぎるときサンプリングしている
            //（グラフ描画の負荷が高くなりすぎるから）
